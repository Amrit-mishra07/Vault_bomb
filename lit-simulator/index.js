...
app.post("/simulate-trigger", (req, res) => {
    const { switchId, journalist, triggerer, arweaveTxId } = req.body;
    
    if (!switchId || !triggerer) {
        return res.status(400).json({ error: "Missing switchId or triggerer" });
    }

    console.log(`\n======================================================`);
    console.log(`🚨 LIT ACTION UNLOCKED FOR ${journalist || "Unknown"} 🚨`);
    ...
    try {
        ...
        res.json({ success: true, proof: publicationProof });
    } catch (e) {
